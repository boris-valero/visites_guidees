<template>
  <div id="intro-vue">
    ...
    <template v-else>
      ...
      <!-- Déplace l'image ici, après les paragraphes -->
      <img v-if="properties.img" :src="resolveImagePath(properties.img)" alt="" />
      <template v-if="properties.choices">
        <div class="links-wrapper">
          <div v-for="(choice, index) in properties.choices" :key="index">
            <NcButton
              :type="choice.success ? 'success' : 'error'"
...
export default {
  ...
  data() {
    return {
      ...
      properties: {
        ...
        links: [],
        choices: [],
        introManager: undefined,
      }, // properties passed by introManager.js
  ...
  computed: {
    linksUrl() {
      return this.properties.links.map((link) =>
        generateUrl(`/apps/${link.linkId}/`)
      );
    },
    ...
    computedLinks() {
      return this.properties.links.filter(
        (link) =>
          this.installedApps.includes(link.linkId) ||
          link.linkId === "user" ||
          link.linkId === "admin"
      );
    },
  ...
  async mounted() {
    if (this.properties.links) {
      try {
        this.installedApps = (
          await axios.get(generateUrl("/apps/intros/apps"))
        ).data.ocs.data.apps;
      } catch (err) {
        this.error = err;
        this.loading = false;
  ...
  methods: {
      ...
      if (imgPath.startsWith('./')) {
        return linkTo('intros', imgPath.substring(2))
      }
      // Sinon retourner le chemin tel quel (URL absolue)
      return imgPath
    },
    /**
